...
const painPoints: PainPoint[] = [
  {
    ...
    summary: "Work is too manual, repetitive, and dependent on individual effort instead of streamlined workflows.",
  },
  {
    id: "integration-disconnected-systems",
    label: "Integration & Disconnected Systems",
    summary: "Teams work across disconnected tools and systems, creating friction, duplicate effort, and handoff issues.",
  },
  {
    id: "inconsistency-standardization",
    label: "Inconsistency & Standardization",
    summary: "Processes, documentation, and execution vary too much across teams, reducing quality and predictability.",
  },
  {
    id: "communication-transparency",
    label: "Communication & Transparency",
    summary: "People do not always have clear visibility into status, ownership, decisions, or expectations.",
  },
  {
    id: "training-onboarding-knowledge",
    label: "Training, Onboarding & Knowledge Access",
    summary: "It is difficult to learn processes, find the right information, and ramp up quickly in a complex organization.",
  },
  {
    id: "complexity-structure",
    label: "Complexity & Organizational Structure",
    summary: "The organization feels complex, layered, or hard to navigate, which slows execution and decision-making.",
  },
  {
    id: "duplication-workarounds",
    label: "Duplication & Workarounds",
    summary: "People create duplicate work, side processes, or manual workarounds because the core process is not working cleanly.",
  },
  {
    id: "frustration-engagement",
    label: "Frustration & Engagement",
    summary: "Pain points create frustration, reduce engagement, and make it harder for teams to do their best work.",
  },
];
...
function scoreIdea(text: string, category: string): ScoreResult {
  ...
  if (words.length < 4) {
    return {
      ...
      explanations: {
        ...
        feasibility: "Not enough detail to determine feasibility.",
        timeToValue: "Not enough detail to estimate timeline.",
  ...
  const explanations = {
    impact:
      impact >= 4
        ? "High likely value with meaningful operational benefit."
        : impact === 3
...
export default function App() {
  ...
  const [statusMessage, setStatusMessage] = useState(
    supabaseEnabled ? "Connected to live session." : "Supabase not configured yet."
  );
  ...
  async function submitIdea() {
    ...
    if (score.isIncomplete) {
      setSubmitError("Please enter a fuller idea before submitting.");
      return;
    }
      ...
      <div style={styles.container}>
        ...
        >
          <section style={{ ...styles.card, ...styles.heroCard }}>
            <div style={styles.cardHeader}>
              <div style={styles.headerRow}>
                ...
                <div>
                  <h1 style={styles.title}>ITBS AI Brainstorm Page</h1>
                  <p style={styles.subtitle}>
                    Enter an AI idea, map it to a pain point, and get an instant score with clear rationale.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div style={styles.cardBody}>
              ...
              <section style={styles.painPointCard}>
                <div style={styles.selectedRow}>
                  ...
                  <div>
                    <div style={styles.eyebrow}>Selected pain point</div>
                    <div style={styles.sectionHeading}>{selectedPainPoint.label}</div>
                    <div style={styles.bodyText}>{selectedPainPoint.summary}</div>
                  </div>
                </div>
              </section>
              ...
              <Field label="AI Brainstorm Idea">
                <textarea
                  ...
                  placeholder="Example: Build an AI assistant that summarizes incoming requests, suggests the right owner, and drafts the first response so teams spend less time triaging emails."
                  style={styles.textarea}
                />
                <div style={styles.helperText}>Tip: press Ctrl/Cmd + Enter to submit.</div>
              </Field>
          ...
          <section style={styles.card}>
            <div style={styles.cardHeader}>
              <h2 style={styles.cardTitle}>Live scoring preview</h2>
              <p style={styles.subtitle}>As participants type, the grader explains how the idea is being evaluated.</p>
            </div>
            <div style={styles.cardBody}>
              {currentPreview ? (
                currentPreview.isIncomplete ? (
                  <div style={styles.incompleteCard}>
                    <div style={styles.incompleteTitle}>Please enter a fuller idea</div>
                    <div style={styles.incompleteBody}>
                      Add what the AI would do, what data or workflow it would use, and who it would help.
                    </div>
                    <div style={styles.incompleteExamples}>
                      Example: &quot;Use AI to summarize ServiceNow tickets and suggest routing for human review.&quot;
                    </div>
                  </div>
                ) : (
        ...
        <div className="bottom-grid" style={styles.bottomGrid}>
          <section style={styles.card}>
            <div style={styles.cardHeader}>
              <div style={styles.headerRow}>
                <div style={{ ...styles.headerIconWrap, ...styles.trophyIconWrap }}>
                  <Trophy size={24} />
                </div>
                <div>
                  <h2 style={styles.cardTitle}>Top 3 ideas</h2>
                  <p style={styles.subtitle}>Automatically ranked by total score.</p>
                </div>
              </div>
            </div>
            <div style={styles.cardBody}>
              {top3.length > 0 ? (
                <div style={styles.stackMd}>
                  {top3.map((item) => (
                    <div key={item.id} style={styles.topIdeaCard}>
                      <div style={styles.betweenRow}>
                        <div>
                          <div style={styles.badgeRow}>
                            <span style={{ ...styles.badge, ...styles.primaryBadge }}>#{item.rank}</span>
                            <span style={styles.badgeMuted}>{item.categoryLabel}</span>
                          </div>
                          <div style={styles.metaText}>{item.teamName}</div>
                          <div style={styles.ideaText}>{item.idea}</div>
                        </div>
                        <div style={styles.scoreColumn}>
                          <div style={styles.metaText}>Score</div>
                          <div style={styles.scoreBig}>{item.score.total}</div>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div style={styles.emptyState}>Submitted ideas will appear here once teams begin entering ideas.</div>
              )}
            </div>
          </section>
          ...
          <section style={styles.card}>
            <div style={styles.cardHeader}>
              <h2 style={styles.cardTitle}>All submissions</h2>
              <p style={styles.subtitle}>Each idea includes its category, total score, and breakdown.</p>
            </div>
            <div style={styles.cardBody}>
                  ...
                  {rankedIdeas.map((item) => (
                    ...
                    >
                      <div style={styles.betweenWrapRow}>
                        <div style={styles.badgeRow}>
                          <span style={{ ...styles.badge, ...styles.successBadge }}>Rank #{item.rank}</span>
                          <span style={styles.badgeMuted}>{item.categoryLabel}</span>
                          <span style={styles.badgeOutline}>{item.teamName}</span>
                        </div>
                        <div style={styles.totalChip}>{item.score.total}/20</div>
                      </div>

                      <div style={styles.ideaText}>{item.idea}</div>
                      <div style={styles.metaText}>Pain point: {item.painPointSummary}</div>

                      <div className="metric-grid" style={styles.metricGrid}>
                        <MiniMetric title="Impact" value={item.score.impact} />
                        <MiniMetric title="Feasibility" value={item.score.feasibility} />
                        <MiniMetric title="Time to Value" value={item.score.timeToValue} />
                        <MiniMetric title="Effort (rev.)" value={item.score.effort} />
                      </div>
                    </motion.div>
                  ))}
                </div>
              ) : (
                <div style={styles.emptyState}>No ideas submitted yet.</div>
              )}
            </div>
          </section>
        </div>
...
function ScoreRow({ label, score, icon: Icon, blurb }: ScoreRowProps) {
  return (
    <div style={styles.scoreRowCard}>
      ...
      <div style={styles.progressTrack}>
        <div style={{ ...styles.progressFill, width: `${score * 20}%` }} />
      </div>
    </div>
  );
}
...
const styles: Record<string, React.CSSProperties> = {
  page: {
    minHeight: "100vh",
    background: "linear-gradient(135deg, #cffafe 0%, #ffffff 45%, #d1fae5 100%)",
    padding: "24px",
  },
  container: {
    maxWidth: "1360px",
  ...
  heroCard: {
    background: "linear-gradient(135deg, rgba(255,255,255,1) 0%, rgba(236,254,255,1) 55%, rgba(236,253,245,1) 100%)",
  },
  cardHeader: {
  ...
  painPointCard: {
    ...
    background: "linear-gradient(90deg, #ecfeff 0%, #ecfdf5 100%)",
    padding: "20px",
  },
  selectedRow: {
    display: "flex",
